...
| `declaration: true` | Emits `.d.ts` files alongside JS output. Required for `@web3nz/shared`, `@web3nz/glyde`, and `@web3nz/agent-sdk` to expose types to consumers. (For apps this is a no-op since their `tsconfig.json` sets `noEmit: true` or doesn't run `tsc` directly.) |
