...
4. Retries the request with an `X-Payment` header containing the signature
...
- A wallet with **USDC** on Avalanche C-Chain (the payment currency)
...
while (true) {
  ...
  if (choice.finish_reason === 'tool_calls' && choice.message.tool_calls) {
    ...
    for (const toolCall of choice.message.tool_calls) {
      ...
      const res = await agent.payAndFetch(url);  // pays automatically
      const data = await res.json();
...
Returns an object with a `payAndFetch` method.
...
Returns `Promise<Response>` — the same as `fetch`.
...
The SDK logs each step to the console:
...
- The private key is only used to sign EIP-712 messages locally — it is never sent over the network
